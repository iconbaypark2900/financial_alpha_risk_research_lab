...
NautilusTrader 1.231.0 is pinned, installed, AND WIRED — see backtest.py, which
runs the engine for real. What that buys, stated at the precision the evidence
supports rather than the precision the library's feature list would allow:

  FR-21  MET, and audited. Data reaches the strategy as timestamped messages in
         time order, so there is no array to index off the end of. `LookAheadAudit`
         checks that on every bar of every run rather than trusting it, and a
         test proves the audit can fail.
  FR-19  PARTIALLY met. Latency and slippage are wired and demonstrated to change
         results. PARTIAL FILLS ARE NOT: they need order-book depth or resting
         limit orders, and market orders against daily bars fill in full even at
         100x the bar's volume. This project has no book data. The limitation is
         pinned by a test rather than left to be discovered.

This paragraph is worded carefully because its previous version was not. It read
"is installed and supplies FR-19 and FR-21" while nothing imported the package —
the Mitiq mistake in its second and quieter form: not hand-rolling a named
library, but booking credit for one that was never called. A requirement is not
satisfied by a line in a dependency file, and it is not satisfied by a library
supporting a feature this project never invokes.

What NautilusTrader does not supply even now that it is wired, and is therefore
